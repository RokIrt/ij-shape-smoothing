...
public class Shape_Smoothing implements ExtendedPlugInFilter, DialogListener {
	...
	private ImagePlus ensureCorrectLUT(ImagePlus imp) {
		if(imp.isInvertedLut()) {
			ImagePlus newImp = duplicateWindow(imp.getProcessor(), imp.getTitle() + " with  a 'non-inverted' LUT");
			...
			YesNoCancelDialog dialog = new YesNoCancelDialog(ImageWindow.getFrames()[0], "Inverting LUT",
					"The LUT of the original image is inverted. This plugin only works with a non-inverted LUT. \n" +
					"The new windows shows the image with a non-inverted lut. Should it be inverted to look like the inverted-LUT image?\n");
	...
	private void doFourierFilter(ImageProcessor ip) {
		...
		if (doAbsoluteThreshold) {
			shapeSmoothingUtil.fourierFilter(ip, thresholdValueAbsolute, false, doOutputDescriptors);
		} else {
			shapeSmoothingUtil.fourierFilter(ip, thresholdValuePercentual, true, doOutputDescriptors);
		}
